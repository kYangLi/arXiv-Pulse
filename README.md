...
**arXiv Pulse** 是一个 Python 包，用于自动化爬取、总结和跟踪 arXiv 在凝聚态物理、密度泛函理论（DFT）、机器学习、力场和相关计算材料科学领域的最新研究论文。提供简化的命令行界面和 Web 界面，通过 **5 个核心命令** 提供专业级的文献管理体验。
...
### Web 界面
- **FastAPI 后端**：高性能异步 API，支持 SSE 实时流式更新
- **Vue 3 + Element Plus 前端**：现代化响应式界面
- **实时进度显示**：SSE 流式展示搜索、同步、翻译进度
...
### 自动处理功能
- **自动翻译**：搜索和更新时自动翻译标题和摘要
- **AI 自动总结**：自动总结论文关键发现，缓存到数据库
- **自动获取图片**：自动获取论文第一个图片，缓存到数据库
...
| 端点 | 方法 | 说明 |
|------|------|------|
| `/api/papers/search/stream` | GET (SSE) | AI 搜索，实时流式返回 |
...
#### `pulse sync` - 同步最新论文到数据库
**功能**：同步最新论文到数据库，不生成报告。支持普通模式和强制模式。

```bash
# 普通模式：按时间从近到早同步，遇到已存在的论文立即停止
pulse sync [目录路径] --years-back 5

# 强制模式：继续查询，跳过已存在的论文，用于扩展数据库
pulse sync [目录路径] --force --years-back 5 --arxiv-max-results 30000
```

**参数**：
- `--years-back`：同步回溯的年数（默认：YEARS_BACK配置，如未配置则为5年）
- `--force`：强制同步：继续查询，跳过已存在的论文（用于扩展数据库）
- `--arxiv-max-results`：arXiv API 最大返回论文数（默认：ARXIV_MAX_RESULTS配置）

**使用场景**：
- **普通模式（无 --force）**：日常更新，只获取最新发布的论文，遇到已存在即停止
- **强制模式（--force）**：扩展数据库（如从2年扩展到5年），继续查询直到达到限制
